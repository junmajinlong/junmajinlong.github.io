...
12. 用户态CPU想要执行特权操作，需要发起系统调用来请求内核帮忙完成对应的操作。其实是**在发起系统调用后，CPU会执行trap指令陷入(trap)到内核**。当特权操作完成后，需要执行一个指令让CPU返回到用户态。

13. 除了系统调用会陷入内核，更多的是硬件会引起trap行为陷入内核，因为所有的硬件都可以在需要的时候通过总线发送硬件中断信号(也称为硬件异常，这个异常并非真的代表硬件出了故障，而是表示一种需要尽快处理的事件)，使得CPU控制权可以回到操作系统，让内核去处理该中断。  

14. 中断指的是打断正常的执行流程，并在需要时恢复正常的执行流程，就像这个突发事件从没有发生过一样，但却实实在在的处理过这个事件。所以，发生中断时需要保证能够去处理中断事件，还要保存中断发生时的正常执行流程，以便后续恢复正常执行流程。  
     - 中断是操作系统中非常重要的机制，分为硬件中断和软件中断。  
     - 软中断可以通过一些特殊的系统调用来实现，比如发生一些信号。  
     - 每个硬件都能在需要的时候通过总线发送硬件中断信号，比如IO操作完成的时候，磁盘可以发送磁盘IO中断。  
     - 同一时刻可能会有很多中断事件的发生，所以中断需要排队，此外中断也有优先级来决定哪种中断比较重要是需要尽快处的。  
     - 当接收到中断后，CPU控制权交给内核，内核开始处理中断，中断处理的最后一个步骤是跳回中断地点继续向下执行，但是有可能在执行此步骤之前就退出了该执行流程，使得不会继续向下执行。  
15. top/ps等工具看到的CPU利用率，并不真的代表CPU的利用率。CPU执行任务时，除了执行指令，很多时候是在等待访问内存来获取相关指令或数据，换句话说，CPU这段时间其实是在闲着，但这段时间也会计入CPU的使用时间。这种类型的等待称为**memory stall**。所以，CPU的真正效率应该看它的IPC(instructions per second)，即每秒执行的指令数量，可通过`perf`命令查看。当IPC小于1(严格地说是小于核数)，可以认为是高速缓存的命中率低，IPC大于1，可以认为是计算密集型任务。参考http://www.brendangregg.com/blog/2017-05-09/cpu-utilization-is-wrong.html  

# 关于CPU的基本组成

1. CPU是用来运算的(加法运算+、乘法运算\*、逻辑运算and not or等)，例如`c=a+b`。  

2. 运算操作涉及到数据输入(input)、处理、数据输出(output)，a和b是输入数据，加法运算是处理，c是输出数据。  

3. CPU需要使用一个叫做**存储器(也就是各种寄存器)**的东西保存输入和输出数据。以下是几种常见的寄存器(前文也介绍了一些)  
...
4. CPU还要将一些常用的基本运算工具(如加法器)放进CPU，这部分负责运算，称为**算术逻辑单元**(ALU, Arithmetic Logic Unit)。  

5. CPU中还有一个**控制器**(CU, Control Unit)，负责将存储器中的数据送到ALU中去做运算，并将运算后的结果存回到存储器中。  
    - 控制器还包含了一些控制信号。  
...
8. 这就是冯诺依曼结构图，也就是现在计算机的结构图。  
     ![](/img/os/1563014520734.png)


# 关于CPU的多核和多线程(简单易理解版)

详细内容见：[关于多CPU、多核和多线程](/os/multi_cpu)  
...
3. **对于超线程CPU来说，每核CPU可以有多个线程(数量是两个，比如1核双线程，2核4线程，4核8线程)，每个线程都是一个虚拟的逻辑CPU(比如windows下是以逻辑处理器的名称称呼的)，而每个线程在OS看来也是独立的CPU**。  
    ...
    ![](/img/os/733013-20190706104845832-630323371.jpg)  
